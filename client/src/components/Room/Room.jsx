    ...
    return (
        <div className='wrapper'>
            <div className='panel back'>
                <div className='list'>
                    {users.map((user,i) => <User key={user.id} name={user.name} isOwner={i==0} />)}
                </div>
                {isOwner && <div onClick={start} className='btn interactive'>Start</div> }
                <div onClick={exitRoom} className='btn interactive'>Exit</div>
            </div>
        </div>
    )
}